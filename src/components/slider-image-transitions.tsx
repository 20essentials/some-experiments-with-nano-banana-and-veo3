'use client';
//@ts-nocheck
import { arrayImages } from '@/lib/const';
import {
  BufferGeometry,
  Color,
  DoubleSide,
...
export function SliderImageTransitions_() {
  ...
  useEffect(() => {
    function App() {
      ...
      function initListeners() {
        ...
        window.addEventListener('wheel', e => {
          e.preventDefault();
          if (e.deltaY > 0) {
            targetProgress = limit(
              targetProgress + 1 / 20,
              0,
              conf.images.length - 1
            );
          } else {
            targetProgress = limit(
              targetProgress - 1 / 20,
      ...
      function updateSize() {
        ...
        if (renderer && camera) {
          ...
          screen.wWidth = wsize[0];
          screen.wHeight = wsize[1];
        }
    ...
    class AnimatedPlane {
      ...
      initAnimAttributes() {
        ...
        this.bGeometry.setAttribute(
          'rotation',
          new InstancedBufferAttribute(rotations, 3)
        );
      }
  ...
  return (
    <canvas id='canvas' ref={canvasRef} className=' w-full h-screen block' />
  );
}

export const SliderImageTransitions = dynamic(
  async () => SliderImageTransitions_,
  { ssr: false }
);
